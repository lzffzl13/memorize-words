...
function renderStatsShell() {
    app.innerHTML = `<div class="page" data-page="stats">
        <h2>学习统计</h2>
        <div id="stats-overview" style="margin-top:20px">
            <div class="stats-cards">
                <div class="card"><div class="num">--</div><div class="label">总词数</div></div>
                <div class="card"><div class="num">--</div><div class="label">已掌握</div></div>
                <div class="card"><div class="num">--</div><div class="label">学习中</div></div>
                <div class="card"><div class="num">--</div><div class="label">待复习</div></div>
                <div class="card"><div class="num">--</div><div class="label">新词</div></div>
            </div>
        </div>
        <div id="stats-daily" style="margin-top:30px"><div class="card" style="padding:20px;color:#999">加载每日趋势中...</div></div>
        <div id="stats-sessions" style="margin-top:30px"><div class="card" style="padding:20px;color:#999">加载练习历史中...</div></div>
        <div id="stats-weak-words" style="margin-top:30px"><div class="card" style="padding:20px;color:#999">加载薄弱词汇中...</div></div>
    </div>`;
}

function renderStatsOverview(overview) {
    return `<div class="stats-cards" style="margin-top:20px">
...
function renderStatsDaily(dailyData) {
    let html = `<h3>每日趋势</h3>
...
function renderStatsSessions(sessions) {
    ...
    if (sessions.length === 0) {
        html += `<div class="card" style="margin-top:16px;text-align:center;color:#999">暂无练习记录</div>`;
        return html;
    }
...
function renderStatsWeakWords(weakWords) {
    let html = `<h3>薄弱词汇 <span style="font-size:14px;color:#999;font-weight:normal">(正确率 < 60%)</span></h3>`;

    if (weakWords.length === 0) {
        html += `<div class="card" style="margin-top:16px;text-align:center;color:#999">暂无薄弱词汇数据，多练习几次就会显示了</div>`;
        return html;
    }
...
async function renderStats() {
    ...
    api("/api/stats/overview").then((overview) => {
        if (!isStatsPage()) return;
        document.getElementById("stats-overview").innerHTML = renderStatsOverview(overview);
    });

    api("/api/stats/daily").then((dailyData) => {
        if (!isStatsPage()) return;
        document.getElementById("stats-daily").innerHTML = renderStatsDaily(dailyData);
    });

    api("/api/stats/sessions?limit=7").then((sessions) => {
        if (!isStatsPage()) return;
        document.getElementById("stats-sessions").innerHTML = renderStatsSessions(sessions);
    });

    api("/api/stats/weak-words?limit=20").then((weakWords) => {
        if (!isStatsPage()) return;
        document.getElementById("stats-weak-words").innerHTML = renderStatsWeakWords(weakWords);
    });
}
